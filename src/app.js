...
function computeItemFit(item, rmin) {
  ...
  const headline = item.type === 'taskbee' ? TASKBEE.line : 'Your brand, same paint, same nine words';
  return fitTextToBlob(ctx, headline, item.type === 'taskbee' ? 'Paid' : 'Open slot', '', rmin, 'standard', Math.min(W, H));
}
...
class Splash {
  ...
  morePillBox() {
    // Small pill centered a bit below the headline — see drawPromptContent.
    const w = this.R * 0.62, h = this.R * 0.18;
    return { x: this.x - w / 2, y: this.y + this.R * 0.22, w, h };
  }
...
// Taskbee / Claim-a-slot — headline + a small explicit "More"/"See slot"
// pill so tap-to-dismiss is never violated by surprise (change-spec v2 §3).
function drawPromptContent(c, s) {
  ...
  c.font = '800 ' + s.pillPx + 'px ' + FONT_UI;
  c.globalAlpha *= 0.85;
  c.fillText(s.item.type === 'taskbee' ? 'SPONSOR ZERO' : 'CLAIM A SLOT', 0, -s.R * 0.42);
  c.globalAlpha = s.alpha;

  c.font = heavyFont(s.fontPx);
  c.shadowColor = 'rgba(0,0,0,0.35)'; c.shadowOffsetY = 2; c.shadowBlur = 3;
  const totalH = s.lines.length * s.lineH;
  const y0 = -totalH / 2 + s.lineH / 2 - s.R * 0.06;
  for (let i = 0; i < s.lines.length; i++) c.fillText(s.lines[i], 0, y0 + i * s.lineH);
  c.shadowColor = 'transparent';

  const box = s.morePillBox();
  const pw = box.w, ph2 = box.h, px = box.x - s.x, py = box.y - s.y;
  c.fillStyle = INK;
  if (c.roundRect) { c.beginPath(); c.roundRect(px, py, pw, ph2, ph2 / 2); c.fill(); }
  else c.fillRect(px, py, pw, ph2);
  c.fillStyle = '#FFFFFF';
  c.font = '800 ' + Math.max(10, ph2 * 0.5) + 'px ' + FONT_UI;
  c.fillText(s.item.type === 'taskbee' ? 'More' : 'See slot', px + pw / 2, py + ph2 / 2 + 1);
}
...
function findPlacement(fit, preferredPos, excludeIdx) {
  ...
  const first = tryPos(preferredPos);
  if (first) return first;
  const marginX = Math.max(fit.R * 0.5, fit.boxW / 2 + 16);
  const marginY = Math.max(fit.R * 0.5, fit.boxH / 2 + 16);
  for (let i = 0; i < 20; i++) {
  ...
  return preferredPos; // exhausted retries — a rare residual overlap beats a stall
}
